 ...
 *
 * $Id: multi.c 5516 2005-08-31 20:56:48Z sarnoud $
 ***************************************************************************/
...
CURL_EXTERN CURLMcode curl_hc_fd_isset(CURLM *multi_handle, 
				       CURL *easy_handle, 
				       fd_set *fds, 
				       int* set) {
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  ...
  /* Can't tolerate any null ptrs as argments */
  if(!easy_handle || !fds || !set) {
    return CURLM_BAD_EASY_HANDLE; /* what's the right error?! */
  }
  ...
  /* scan through the list to find the 'curl_handle' */
  easy = multi->easy.next;
  while(easy) {
    if(easy->easy_handle == (struct SessionHandle *)easy_handle)
      break;
    easy=easy->next;
  }
  if(!easy) {
